...
const NurseList = ({ searchTerm, setSearchTerm, setShowAddForm, setSelectedNavigator, setShowAssignedStudents, refreshKey = 0 }) => {
  ...
  const fetchNurses = async (filterParams = {}) => {
    try {
      setIsLoading(true);
      setError(null);
      
      const response = await nursesService.getNurses({
        search: searchTerm,
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this nurse?')) {
      try {
        ...
        showSnackbar('Error deleting nurse', 'error');
      }
    }
  };
  ...
  const filteredNurses = nurses.filter(nurse => {
    ...
    // Gender filter
    const matchesGender = !filters.gender || nurse.gender === filters.gender.value;

    // Languages filter
    const matchesLanguages = !filters.languages || 
      filters.languages.every(lang => nurse.languagesSpoken.includes(lang.value));

    // City filter
    const matchesCity = !filters.city || nurse.city === filters.city.value;

    // Area filter
    const matchesArea = !filters.area || nurse.area === filters.area.value;

    // Experience range filter
    const experience = nurse.experience || 0;
  ...
  const handleViewProfile = (nurse) => {
    setSelectedNavigator({
      ...nurse,
      role: 'nurse' // Ensure role is set for NavigatorDetail component
    });
  };
  ...
  return (
    <>
      <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-4">
        <h2 className="text-2xl font-semibold">Nurses</h2>
      ...
      <div className="text-sm text-gray-500">
        Showing {nurses.length} of {pagination.total} {pagination.total === 1 ? 'nurse' : 'nurses'}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {nurses.length === 0 && !isLoading && (
            <div className="col-span-full flex items-center justify-center py-12 bg-gray-50 rounded-xl">
              <p className="text-gray-500 text-lg">No nurses found matching your search.</p>
            </div>
          )}
        </div>
      )}
